...
import json
import httpx
import tomllib as toml  # Use tomllib for better compatibility
...
class LLMClient:
        ...
        **kwargs,
        ...
        try:
            response = await self.client.post(
                self.base_url, json=payload, headers=headers, timeout=180.0
            )
            ...
            if response.status_code != 200:
                ...
                console.print(
                    f"Status code: {error_details['status_code']}", style="red"
                )
                console.print(f"URL: {error_details['url']}", style="red")
                console.print(
                    f"Provider: {error_details['config']['provider']}", style="red"
                )
                console.print(
                    f"Model: {error_details['config']['model_id']}", style="red"
                )
                console.print("Response content:", style="red")
                print_json(
                    data=error_details.get(
                        "response_json", error_details.get("response_text", "")
                    ),
                    indent=2,
                )
            ...
            console.print(
                f"Please check network connection and API server status", style="yellow"
            )
            ...
            LLMClient._global_api_error_count += 1
            error_msg = (
                f"{self.config.provider} API HTTP error: {e.response.status_code}"
            )
            ...
            console.print(
                f"Request URL: {self.base_url}/chat/completions", style="yellow"
            )
    ...
    def get_api_stats(self) -> Dict[str, int]:
        ...
        return {
            ...
            "success_rate": (
                round(
                    LLMClient._global_api_success_count
                    / LLMClient._global_api_call_count
                    * 100,
                    2,
                )
                if LLMClient._global_api_call_count > 0
                else 0.0
            ),
        }
...
def load_config(
    config_path: str = ".configs.toml", provider: str = "vllm"
) -> LLMConfig:
...
class RoTKChatAgent:
    ...
    def __init__(
        self, llm_config: LLMConfig, faction: str = "wei", system_prompt: str = ""
    ):
    ...
    async def _process_turn_start_if_available(self, context_name: str = "") -> bool:
        ...
        try:
            ...
            if isinstance(turn_evt, dict):
                ...
                if evt_faction == str(self.faction).lower() and isinstance(
                    evt_turn, int
                ):
                    if evt_turn > self._last_turn_notified:
                        ...
                        if self._turn_gate.is_set():
                            console.print(
                                f"⚠️ Found turn_start but gate is already open - likely a stale event (evt_turn={evt_turn}, last={self._last_turn_notified}) [{context_name}]",
                                style="yellow",
                            )
                            return False
                        ...
                        console.print(
                            f"📣 Injected turn_start hint for faction={self.faction}, turn={evt_turn} [{context_name}]",
                            style="green",
                        )
                        ...
                        if context_name:
                            console.print(
                                f"⏳ Detected turn_start but not newer (evt_turn={evt_turn}, last={self._last_turn_notified}) [{context_name}]",
                                style="dim yellow",
                            )
                else:
                    if (
                        evt_faction
                        and evt_faction != str(self.faction).lower()
                        and context_name
                    ):
                        console.print(
                            f"⏳ Detected turn_start for other faction: {evt_faction} [{context_name}]",
                            style="dim yellow",
                        )
            return False
        except Exception as e:
            console.print(
                f"⚠️ Turn-start processing failed [{context_name}]: {e}", style="yellow"
            )
            return False
            ...
            for match in matches:
                ...
                try:
                    ...
                    if function_name:
                        ...
                        console.print(
                            f"📝 Parsed tool call: {function_name} with args: {arguments}",
                            style="cyan",
                        )

                except json.JSONDecodeError as e:
                    console.print(
                        f"⚠️ Found text-based tool call with content", style="red"
                    )
                    console.print(
                        f"⚠️ Failed to parse tool call JSON: {json_str} - {e}",
                        style="red",
                    )
                    return True
                except Exception as e:
                    console.print(
                        f"⚠️ Error parsing text-based tool calls: {e}", style="red"
                    )
                    return True
    ...
    async def _handle_tool_calls(self, tool_calls: List[Dict[str, Any]]):
        ...
        if parallel_execution:
            console.print(
                "⚡ Multiple perform_action calls detected, using parallel execution mode",
                style="cyan",
            )
    ...
    async def _execute_single_tool_call(self, tool_call: Dict[str, Any]):
        ...
        console.print(
            f"╭───────────────────────────────── Executing tool '{function_name}' with arguments ───────────────────────────────────╮",
            style="magenta",
        )
        ...
        try:
            ...
            if "params" in arguments and isinstance(arguments["params"], str):
                ...
                try:
                    ...
                    raise ValueError(
                        f"LLM generated invalid JSON string for 'params': {arguments['params']}. Error: {e}"
                    )
    ...
    def _contains_strategy_keywords(self, text: str) -> bool:
        ...
        for sw in structure_terms:
            search_text = (
                text if any(c >= "\u4e00" for c in sw) else t
            )  # Chinese or English
    ...
    def _contains_strategy_sequence(self, text: str) -> bool:
        ...
        def has_terms_without_negation(
            seg: str, terms_zh: list[str], terms_en: list[str]
        ) -> bool:
            s = seg.lower()
            # Check for terms
            found_terms = []
        ...
        for i in range(len(segments)):
            ...
            for j in range(i + 1, min(i + 3, len(segments))):
                ...
                if has_terms_without_negation(
                    a, move_terms_zh, move_terms_en
                ) and has_terms_without_negation(b, attack_terms_zh, attack_terms_en):
                    return True
                # Attack -> move
                if has_terms_without_negation(
                    a, attack_terms_zh, attack_terms_en
                ) and has_terms_without_negation(b, move_terms_zh, move_terms_en):
                    return True
    ...
    def _filter_tool_result(self, function_name: str, result: Any) -> Any:
        ...
        if function_name == "perform_action":
            if (
                "visible_environment" in filtered_result
                and "unit_info" in filtered_result
            ):
                filtered_result = self._filter_observation_result(filtered_result)
            elif (
                "faction" in filtered_result
                and "units" in filtered_result
                and "total_units" in filtered_result
            ):
                filtered_result = self._filter_faction_state_result(filtered_result)
            elif "details" in filtered_result and (
                "moved successfully" in str(filtered_result.get("details", ""))
                or "failure_reason" in filtered_result
            ):
                filtered_result = self._filter_move_result(filtered_result)
            elif (
                "battle_summary" in filtered_result
                and "casualties_inflicted" in filtered_result
            ):
    ...
    def _filter_observation_result(self, result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "unit_info" in filtered_result and isinstance(
            filtered_result["unit_info"], dict
        ):
            ...
            if "capabilities" in unit_info and isinstance(
                unit_info["capabilities"], dict
            ):
        ...
        if "visible_environment" in filtered_result and isinstance(
            filtered_result["visible_environment"], list
        ):
            filtered_env = []
            for tile in filtered_result["visible_environment"]:
                if isinstance(tile, dict):
                    ...
                    if (
                        isinstance(movement_access, dict)
                        and "reachable" in movement_access
                    ):
                    ...
                    if (
                        ...
                        and "in_attack_range" in attack_info
                    ):
        ...
        if "units" in filtered_result and isinstance(filtered_result["units"], list):
            ...
            for unit in filtered_result["units"]:
                if isinstance(unit, dict):
                    ...
                    if "unit_status" in filtered_unit and isinstance(
                        filtered_unit["unit_status"], dict
                    ):
                    ...
                    if "capabilities" in filtered_unit and isinstance(
                        filtered_unit["capabilities"], dict
                    ):
    ...
    def _filter_move_result(self, result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not result.get("result", True):
            if "suggested_action" in result:
                ...
                filtered_result = {
                    k: v for k, v in result.items() if k in essential_keys
                }
                return filtered_result
    ...
    def _filter_attack_result(self, result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if result.get("result", True):
            ...
            if "battle_summary" in result and isinstance(
                result["battle_summary"], dict
            ):
                ...
                if "battle_result" in battle_summary and isinstance(
                    battle_summary["battle_result"], dict
                ):
                    battle_result = battle_summary["battle_result"]
                    filtered_battle["battle_result"] = {
                        k: battle_result.get(k)
                        for k in [
                            "is_critical",
                            "damage_dealt",
                            "target_destroyed",
            ...
            if "tactical_info" in result and isinstance(result["tactical_info"], dict):
                ...
                if "attack_was_effective" in tactical:
                    filtered_result["attack_was_effective"] = tactical[
                        "attack_was_effective"
                    ]
                if "target_strength_percentage" in tactical:
                    filtered_result["target_remaining_manpower"] = (
    ...
    async def _register_agent_info(self):
        ...
        try:
            ...
            result = await self.tool_manager.execute_tool(
                "perform_action",
                {"action": "register_agent_info", "params": registration_params},
            )
            if result.get("success"):
                console.print(
                    f"✅ Agent information registered successfully: {self.faction} faction - {config.provider}:{config.model_id} (thinking: {config.enable_thinking})",
                    style="cyan",
                )
            else:
                console.print(
                    f"⚠️ Agent information registration failed: {result.get('message', 'unknown error')}",
                    style="red",
                )
    ...
    async def _report_llm_stats(self):
        ...
        try:
            ...
            if result.get("success"):
                ...
                console.print(
                    f"⚠️ LLM statistics reported failed: {result.get('message', 'unknown error')}",
                    style="red",
                )
    ...
    async def chat(
        ...
        self.conversation_history.append(Message(role="user", content=user_prompt))
        ...
        while iterations < self.max_iterations:
            ...
            try:
                ...
                if "game_ended" in status:
                    console.print(
                        f"🔍 Status check (iteration {iterations}): {status}",
                        style="dim cyan",
                    )
                if status.get("game_ended", False):
                    console.print(
                        f"🏁 Game ended @iteration {iterations}, preparing to report LLM stats and exit",
                        style="yellow bold",
                    )
                ...
                console.print(
                    f"⚠️ Error checking game status: {status_error}", style="red"
                )
            ...
            try:
                ...
                if len(self.conversation_history) > 40:
                    ...
                    console.print(
                        "🧹 Context overflow detected, history has been trimmed and continued",
                        style="cyan",
                    )
                ...
                console.print(
                    f"╭─────────────────────────────────────────────────────── LLM response @iteration {iterations}: ────────────────────────────────────────────────────────╮",
                    style="yellow",
                )
                console.print(
                ...
                console.print(
                    "🔍 Attempting strategy detection @iteration {iterations}",
                    style="cyan",
                )
                asyncio.create_task(
                    self._async_strategy_detection(message.get("content", ""))
                )
                ...
                if not tool_calls_to_use and message.get("content"):
                    console.print(
                        f"🔧 Detecting text-based tool calls in content @iteration {iterations}: {message['content']}",
                        style="cyan",
                    )
                    parsed_tool_calls = self._parse_text_based_tool_calls(
                        message["content"]
                    )
                    if parsed_tool_calls:
                        console.print(
                            "🔧 Detected text-based tool calls, converting to standard format @iteration {iterations}",
                            style="cyan",
                        )
                        self.conversation_history.append(
                            Message(
                        ...
                        console.print(
                            "🔧 Undetected text-based tool calls. @iteration {iterations}",
                            style="cyan",
                        )
                ...
                if message.get("tool_calls"):
                    console.print(
                        f"🔧 Handling tool calls @iteration {iterations}: {message['tool_calls']}",
                        style="cyan",
                    )
                ...
                if finish_reason in ("content_filter"):
                    print(
                        f"success: True, response: {message.get('content', '')}, iterations: {iterations}, finish_reason: {finish_reason}"
                    )
                    break

                # 5) an unexpected finish reason
                console.print(
                    f"Unexpected finish reason @iteration {iterations}: {finish_reason}",
                    style="red",
                )
                return {
                    "success": False,
                ...
                if _is_context_overflow_error(e, error_details):
                    ...
                    console.print(
                        "🧹 Context overflow error detected, history has been trimmed and continued",
                        style="cyan",
                    )
                    continue
        ...
        def on_message(data):
            ...
            if msg_type == "action":
                ...
                current_status.update(
                    {"self_status": {f"Task{msg_data['id']}": outcome}}
                )
                ...
                console.print(
                    "🏁 Received game end notification, preparing to report LLM stats and exit",
                    style="yellow bold",
                )
                ...
                RemoteContext.set_status(current_status)
                console.print(
                    f"🔧 State updated: {current_status}", style="cyan"
                )  # 🆕 Debug information
                ...
                RemoteContext.set_status(current_status)
                console.print(
                    f"📬 Received turn_start: {current_status.get('turn_start')}",
                    style="cyan",
                )
...
- 多用perform_action: "arguments": "{{"action":"get_faction_state","params":{{"faction":"wei"|"shu"|"wu"}}}}"了解当前敌我态势，然后调动所有单位积极进攻，消灭敌人。
        ...
        while True:
            ...
            try:
                await asyncio.create_task(
                    create_agent(faction, system_prompt, user_prompt)
                )
    ...
    async def run_interactive_demo(self):
        ...
        console_system.print(
            "🎯 You can manually control the Agent to perform various actions",
            style="cyan",
        )
        console_system.print("=" * 50)
...
def create_error_details(exception: Exception, **extra_context) -> Dict[str, Any]:
    ...
    if isinstance(exception, httpx.HTTPStatusError):
        ...
        error_details["request_url"] = (
            str(exception.request.url)
        ...
        error_details["json_error"] = (
            "JSON parsing error, maybe the API return format is incorrect"
        )
...
def log_error_to_file(
    ...
    if display_console:
        ...
        if "http_status_code" in error_details:
            ...
            console.print(
                f"📥 Response body: {error_details['response_body'][:500]}...",
                style="yellow",
            )
...
async def get_env_response(request_id, timeout_seconds: float = 60.0):
    ...
    console.print(
        f"⏳ Waiting for ENV response ID: {request_id}, timeout set to: {timeout_seconds}s",
        style="cyan",
    )
    ...
    while True:
        ...
        if elapsed >= timeout_seconds:
            ...
            timeout_error = TimeoutError(
                f"ENV response timeout: ID {request_id}, timeout time: {timeout_seconds}s"
            )
...
async def perform_action(action: str, params: Any):
    ...
    if action == "end_turn":
        raise ValueError(
            "Invalid tool usage: 'end_turn' must be called via the standalone 'end_turn' tool, not 'perform_action'."
        )
    ...
    try:
        ...
        if delay_time > 0:
            console.print(
                f"⏳ Waiting for {delay_time}s to complete the action...", style="cyan"
            )
...
async def create_agent(
    faction: str = "wei", system_prompt: str = "", user_prompt: str = ""
):
    # Load configuration and create independent chat agent
    try:
        config_path = os.path.join(os.getcwd(), ".configs.toml")
        console_system.print(
            f"Found configuration file in current working directory: {config_path}"
        )
        ...
        async def end_turn():
            ...
            try:
                ...
                # return response
                return {
                    "result": "The current turn is over. Wait for the next turn_start to resume."
                }
            except Exception as e:
...
def _calculate_move_delay(params: Any, response: Any) -> float:
    ...
    try:
        ...
        if isinstance(response, dict) and "movement_details" in response:
            estimated_duration = response["movement_details"].get(
