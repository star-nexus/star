...
- 目标：利用兵种特性与战场状态，消灭所有 **$opponent_name ($opponent)** 敌方单位。
- 游戏为 **回合制**：双方轮流行动。
...
- 回合制结束回合用独立 tool `end_turn`（空参数），不要塞进 `perform_action`。
...
AP 和 MP 在 `end_turn` 后新回合开始时完全恢复。
`get_faction_state` 不消耗 AP/MP，对方回合也可使用。
...
在尚未发现敌人时，以敌方 home_base 坐标作为推进目标，并用 hex distance 判断移动是否真正接近目标。
