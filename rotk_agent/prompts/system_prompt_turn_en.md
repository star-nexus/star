...
- End the turn with the dedicated `end_turn` tool (empty args). Do not nest it inside `perform_action`.
...
## 6. AP / MP
### AP
- Each unit has 2 AP per turn (two attacks).
- `attack` costs 1 AP.
...
AP and MP fully restore after `end_turn` when the new turn begins.
`get_faction_state` costs no AP/MP and may be used on the opponent's turn.
