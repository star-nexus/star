...
### Tools
- **end_turn**: End the current turn and restore AP/MP. Parameters is an empty object. Dedicated turn-based tool — do not nest it inside `perform_action`.
...
**Action Points (AP):**
- Each unit has **2 AP** per turn for combat actions (two attack opportunities).
...
- `get_faction_state` does not consume AP or MP and can be used at any time, including during the opponent's turn.
