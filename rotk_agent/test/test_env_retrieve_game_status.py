...
async def test_env_retrieve_game_status(
    server_url: str = "ws://localhost:8000/ws/metaverse",
    env_id: str = "env_1",
    agent_id: str = "agent_test_register_info",
):
    """
    向环境直接发送 retrieve_game_status 动作，并获取 ENV 返回结果。
    ...
    try:
        ...
        # 直接发送 retrieve_game_status 动作
        req_id = await client.send_action("retrieve_game_status", params)
        outcome = await waiter.wait(req_id, timeout=15)

        print("retrieve_game_status =>", json.dumps(outcome, ensure_ascii=False, indent=2))
        return outcome
    finally:
...
if __name__ == "__main__":
    asyncio.run(test_env_retrieve_game_status())
