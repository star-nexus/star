...
async def test_env_register_agent_info(
    ...
    try:
        params = {
            ...
            "agent_id": agent_id,
            "version": "1.0.0",
            "note": "Agent using openai",
            "enable_thinking": False,
        }

        req_id = await client.send_action("register_agent_info", params)
        outcome = await waiter.wait(req_id, timeout=15)

        print("register_agent_info =>", json.dumps(outcome, ensure_ascii=False, indent=2))
        return outcome
    finally:
...
if __name__ == "__main__":
    asyncio.run(test_env_register_agent_info())
