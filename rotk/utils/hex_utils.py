...
class HexConverter:
    ...
    def get_hex_corners(self, q: int, r: int) -> List[Tuple[float, float]]:
        """获取六边形的6个顶点坐标 - 精确的pointy-top布局"""
        center_x, center_y = self.hex_to_pixel(q, r)
        corners = []

        # pointy-top布局：从-30度开始，每次旋转60度
        # 使用精确的角度计算
        for i in range(6):
            angle_deg = 60 * i - 30
