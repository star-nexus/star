...
class UnitSystem(System):
    ...
    def __init__(self):
        super().__init__(
            [UnitStatsComponent, PrecisePositionComponent, UnitRenderComponent],
            priority=15,
        )
    ...
    def update(self, world: World, delta_time: float) -> None:
        ...
        moving_units = world.get_entities_with_components(
            PrecisePositionComponent, UnitStateComponent, UnitMovementComponent
        )
        for entity in moving_units:
            state_comp = world.get_component(entity, UnitStateComponent)
            pos_comp = world.get_component(entity, PrecisePositionComponent)
            ...
            if state_comp.state == UnitState.MOVING and state_comp.target_position:
                ...
                if distance < 0.1:  # 足够接近目标
                    ...
                    # 继续移动
                    speed = (
                        move_comp.current_speed * delta_time / 5.0
                    )  # 调整以获得合适的移动速度
                    ...
                    # 更新位置
                    pos_comp.grid_x += move_dir_x * speed
                    pos_comp.grid_y += move_dir_y * speed

                    # 如果跨越格子边界，更新格子坐标和偏移
                    new_grid_x = int(pos_comp.grid_x)
                    new_grid_y = int(pos_comp.grid_y)

                    if new_grid_x != pos_comp.grid_x or new_grid_y != pos_comp.grid_y:
                        pos_comp.offset_x = pos_comp.grid_x - new_grid_x
                        pos_comp.offset_y = pos_comp.grid_y - new_grid_y
                        pos_comp.grid_x = new_grid_x
                        pos_comp.grid_y = new_grid_y
    ...
    def _handle_move_command(self, message: Message) -> None:
        ...
        # 获取单位组件
        pos_comp = self.world.get_component(unit_entity, PrecisePositionComponent)
