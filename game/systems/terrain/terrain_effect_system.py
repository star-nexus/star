...
class TerrainEffectSystem(System):
    ...
    def __init__(
        ...
        self.terrain_effects = {
            TerrainType.RIVER: self._apply_water_effect,  # 水：减少生命值，移速降低
            # 水边：静止恢复体力（河流、湖泊、海洋周围的地形）
            # 这个需要特殊处理，因为不是直接对应地形类型
            # 城市：占领后范围内攻击力增加，同时只能由一个阵营占领生效，离开和杀死对方后失效
            # 城市目前没有对应的地形类型，需要在TileComponent中添加标记
            TerrainType.MOUNTAIN: self._apply_mountain_effect,  # 山地：骑兵移速速度降为最低，静止时隐蔽，对非山地地形攻击加成
            TerrainType.FOREST: self._apply_forest_effect,  # 森林：静止时可以隐蔽
            TerrainType.PLAIN: self._apply_plain_effect,  # 平原：骑兵加速
        }
        # 用于跟踪单位上一次所在的地形
    ...
    def update(self, delta_time):
        ...
        for unit_entity, (unit_component,) in self.context.with_all(
            ...
            # 确保单位有UnitEffectComponent
            if not self.context.component_manager.has_component(
                unit_entity, UnitEffectComponent
            ):
                self.context.component_manager.add_component(
                    unit_entity, UnitEffectComponent()
                )

            unit_effect = self.context.component_manager.get_component(
                unit_entity, UnitEffectComponent
            )
    ...
    def _get_map(self):
        """获取地图实体和组件"""
        entity = self.context.with_all(MapComponent).first()
        map_component = self.context.get_component(entity, MapComponent)
    ...
    def _get_tile_at_position(
        ...
        # 将世界坐标转换为格子坐标
        tile_x = int(x / map_component.tile_size)
        tile_y = int(y / map_component.tile_size)
    ...
    def _is_near_water(self, map_component: MapComponent, x: float, y: float) -> bool:
        ...
        for dx in [-1, 0, 1]:
            for dy in [-1, 0, 1]:
                ...
                if 0 <= nx < map_component.width and 0 <= ny < map_component.height:
                    tile_entity = map_component.tile_entities.get((nx, ny))
                    if tile_entity:
                        tile_component = self.context.component_manager.get_component(
                            tile_entity, TileComponent
                        )
