"""
MOBA地图生成器 - 扩展MapSystem以支持MOBA风格地图
MOBA Map Generator - Extension for MapSystem to support MOBA-style maps
"""
...
class MOBAMapMixin:
    """MOBA地图生成混入类，为MapSystem添加MOBA地图生成功能"""
    ...
    def _generate_moba_map(self):
        ...
        print("[MOBA Map] 🏟️ 开始生成MOBA风格地图...")

        # 先填充基础地形
        for q in range(-map_radius, map_radius + 1):
            for r in range(-map_radius, map_radius + 1):
                if abs(q + r) <= map_radius:  # 六边形边界内
                    # 默认为野区地形
        ...
        # 定义MOBA地图的关键结构
        self._create_moba_lanes(map_data)       # 创建三条兵线
        self._create_moba_river(map_data)       # 创建中央河流
        self._create_moba_bases(map_data)       # 创建队伍基地
        self._create_moba_towers(map_data)      # 创建防御塔
        self._create_moba_jungle_areas(map_data) # 完善野区布局

        # 设置为单例组件
        self.world.add_singleton_component(map_data)
        
        print("[MOBA Map] ✅ MOBA地图生成完成!")
        self._print_moba_map_summary()

    def _create_moba_lanes(self, map_data: MapData):
        """创建三条MOBA兵线路径"""
        print("[MOBA Map] 🛣️ 创建三条兵线...")
    ...
    def _create_moba_bases(self, map_data: MapData):
        ...
        for ancient_pos in [team1_ancient, team2_ancient]:
            q, r = ancient_pos
            if abs(q + r) <= 9:  # 稍微扩展边界以容纳主堡
                if (q, r) not in map_data.tiles:
                    # 创建新的主堡地块
                    tile_entity = self.world.create_entity()
                    self.world.add_component(tile_entity, HexPosition(q, r))
                    # 使用CITY类型代替ANCIENT（因为ANCIENT可能没有纹理）
