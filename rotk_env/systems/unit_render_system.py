"""
修改后的unit_render_system.py - 添加详细性能分析
"""
...
class UnitRenderSystem(System):
    ...
    def _load_unit_textures(self) -> None:
        ...
        if len(self.unit_textures) > 0:
            ...
            print(f"[整合版] 成功加载 {len(self.unit_textures)} 个单位贴图，预缓存 {len(self.scaled_texture_cache)} 个尺寸变体")
        else:
    ...
    def update(self, delta_time: float) -> None:
        ...
        if self.enable_profiler and update_start:
            ...
            if self.render_count % self.profiler_interval == 0:
                self._print_detailed_performance_stats(len(visible_units), render_strategy, total_time)
    ...
    def _get_visible_units(self, camera_offset: List[float], zoom: float) -> List[int]:
        ...
        entities = list(self.world.query().with_all(HexPosition, Unit, UnitCount).entities())
        if self.enable_profiler:
    ...
    def _render_units_full_featured(self, visible_units: List[int], camera_offset: List[float], zoom: float):
        """完整功能的单位渲染"""
        ...
        for pos_key, units in units_by_position.items():
            ...
            self._render_unit_group_full(pos_key, units, camera_offset, zoom, animation_system)
            if self.enable_profiler:
    ...
    def _render_unit_group_full(self, pos_key, units, camera_offset, zoom, animation_system):
        """完整功能的单位组渲染"""
        ...
        # 获取基础位置
        base_world_x, base_world_y = self.hex_converter.hex_to_pixel(pos_key[0], pos_key[1])
        ...
        if total_factions == 1:
            ...
            self._render_same_faction_units(faction_units, base_screen_x, base_screen_y, zoom, animation_system)
        else:
            # 多个阵营：分两半，每半各自等分排列
            self._render_multi_faction_units(units_by_faction, base_screen_x, base_screen_y, zoom, animation_system)
    ...
    def _render_same_faction_units(self, units, base_x, base_y, zoom, animation_system):
        ...
        if unit_count == 1:
            # 只有一个单位，正常渲染在中心
            self._render_single_unit_full(units[0], base_x, base_y, zoom, animation_system)
        else:
            # 布局计算计时（仅在启用profiler时）
            layout_start = time.time() if self.enable_profiler else None
            positions = self._calculate_unit_positions_in_hex(unit_count, base_x, base_y, zoom)
            if self.enable_profiler:
    ...
    def _render_single_unit_full(self, entity, screen_x, screen_y, zoom, animation_system):
        """完整功能的单个单位渲染"""
        position = self.world.get_component(entity, HexPosition)
        unit = self.world.get_component(entity, Unit)
        ...
        texture_size = int(GameConfig.HEX_SIZE * zoom)
        ...
        self._render_unit_count_bar(
            screen_x, screen_y, unit_count, unit_radius, zoom, scale=1.0
        )

        # 绘制单位类型图标
        self._render_unit_icon(screen_x, screen_y, unit, zoom, scale=1.0)
    ...
    #         position = self.world.get_component(entity, HexPosition)
    #         if (position and position.col == target_position.col and 
    #             position.row == target_position.row):
    ...
    def _print_detailed_performance_stats(self, visible_count: int, strategy: str, total_time: float):
        """打印详细的性能统计"""
        print("\n" + "="*80)
        print(f"[UnitRenderSystem] 详细性能分析 - 可见单位: {visible_count}, 策略: {strategy}")
