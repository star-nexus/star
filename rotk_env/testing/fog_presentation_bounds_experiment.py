"""Harness adapter for Fog presentation-bounds feasibility workloads."""
...
from .fog_presentation_bounds_feasibility import PresentationBoundsCollector

EXPERIMENT_ID = "fog_presentation_bounds_feasibility_v1"
...
def install_fog_presentation_bounds_experiment(harness, world, profiler) -> bool:
    """Install isolated start/status/stop commands on the scale harness."""
    del profiler
    if bool(getattr(harness, "_fog_presentation_bounds_experiment_installed", False)):
        return True
    ...
    harness._fog_presentation_bounds_experiment_state = None
    ...
    def _restore(state: Dict[str, Any], presenter, camera) -> Dict[str, bool]:
        ...
        return {
            ...
            "geometry_path_restored": (
                bool(snapshot.get("fused_transform_bounds_enabled"))
                == state["fused_transform_bounds_enabled_before"]
            ),
            "corner_path_restored": (
                bool(snapshot.get("precomputed_hex_corner_offsets_enabled"))
                == state["precomputed_hex_corner_offsets_enabled_before"]
            ),
            "world_corner_path_restored": (
                bool(snapshot.get("tile_world_corner_cache_enabled"))
                == state["tile_world_corner_cache_enabled_before"]
            ),
        }
    ...
    def _start(command: Dict[str, Any]) -> Dict[str, Any]:
        if isinstance(
            getattr(harness, "_fog_presentation_bounds_experiment_state", None),
            dict,
        ):
            return {"ok": False, "error": "fog_presentation_bounds_already_active"}
        for active_name in (
        ...
        direction_name = str(command.get("direction", "horizontal_positive")).strip().lower()
        if mode == "pan" and direction_name not in DIRECTIONS:
            return {"ok": False, "error": "invalid_direction", "directions": sorted(DIRECTIONS)}
        try:
        ...
        if active_units:
            return {"ok": False, "error": "units_must_be_stationary", "active_moving_units": active_units}
        ...
        collector = PresentationBoundsCollector(presenter.renderer.hex_converter)
        state: Dict[str, Any] = {
            "active": True,
            "completed": False,
            ...
            "start_camera": {"offset_x": start_x, "offset_y": start_y, "zoom": start_zoom},
            "original_camera": {
            ...
            "fused_transform_bounds_enabled_before": bool(snapshot["fused_transform_bounds_enabled"]),
            "precomputed_hex_corner_offsets_enabled_before": bool(snapshot["precomputed_hex_corner_offsets_enabled"]),
            "tile_world_corner_cache_enabled_before": bool(snapshot["tile_world_corner_cache_enabled"]),
        }
        for field, _setter in _TIMER_FIELDS:
        ...
        presenter.reset()
        harness._fog_presentation_bounds_experiment_state = state
        return {
            "ok": True,
            ...
            "geometry_path_effective": presenter.diagnostic_snapshot()["geometry_path"],
            "corner_path_effective": presenter.diagnostic_snapshot()["corner_path"],
            "world_corner_path_effective": presenter.diagnostic_snapshot()["tile_world_corner_path"],
            "performance_timers_enabled": False,
        }

    def _status() -> Dict[str, Any]:
        state = getattr(harness, "_fog_presentation_bounds_experiment_state", None)
    ...
    def _stop() -> Dict[str, Any]:
        state = getattr(harness, "_fog_presentation_bounds_experiment_state", None)
        if not isinstance(state, dict):
            return {"ok": False, "error": "fog_presentation_bounds_not_active"}
        camera = _camera()
        presenter = _find_fog_presenter(world)
        end_camera = (
            {"offset_x": float(camera.offset_x), "offset_y": float(camera.offset_y), "zoom": float(camera.zoom)}
            if camera is not None
            else None
        )
    ...
    def _handle(command: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if operation == "start_fog_presentation_bounds_feasibility":
            return _start(command)
        if operation == "fog_presentation_bounds_feasibility_status":
            return _status()
        if operation == "stop_fog_presentation_bounds_feasibility":
            return _stop()
    ...
    if callable(original_update):
        def _update(delta_time: float) -> None:
            ...
            state = getattr(harness, "_fog_presentation_bounds_experiment_state", None)
            if not isinstance(state, dict) or not state["active"] or state["completed"] or not state["anchor_ready"]:
                return
            active_units = _moving_units()
            state["max_active_moving_units"] = max(state["max_active_moving_units"], active_units)
            fog = world.get_singleton_component(FogOfWar)
            if active_units or fog is None or not fog.enabled:
                ...
                state["aborted_reason"] = "units_became_active" if active_units else "fog_became_disabled"
            ...
            if state["target_reached"]:
                # The renderer/observer ran before this harness update, so the
                # final target camera now has canonical bounds evidence.
                state["completed"] = True
                return
            before = (float(camera.offset_x), float(camera.offset_y), float(camera.zoom))
            if state["mode"] == "pan":
                direction_x, direction_y = state["direction_vector"]
                start = state["start_camera"]
                distances = [
                    abs(float(camera.offset_x) - start["offset_x"]) if direction_x else 0.0,
                    abs(float(camera.offset_y) - start["offset_y"]) if direction_y else 0.0,
                ]
                traveled = max(distances)
                remaining = max(0.0, state["target_pixels"] - traveled)
                speed = max(float(camera.speed), 1e-12)
                camera_dt = min(max(0.0, float(delta_time)), remaining / speed)
                delta = speed * camera_dt
                camera.move(direction_x * delta, direction_y * delta)
                after_distances = [
                    abs(float(camera.offset_x) - start["offset_x"]) if direction_x else state["target_pixels"],
                    abs(float(camera.offset_y) - start["offset_y"]) if direction_y else state["target_pixels"],
                ]
                state["target_reached"] = all(
                    value >= state["target_pixels"] or math.isclose(value, state["target_pixels"], rel_tol=0.0, abs_tol=1e-9)
                    for value, direction in zip(after_distances, (direction_x, direction_y))
                    if direction
                )
            else:
                ...
                if target > camera.zoom:
                    camera.zoom = min(camera.zoom + zoom_delta, target)
                else:
                    camera.zoom = max(camera.zoom - zoom_delta, target)
                state["target_reached"] = math.isclose(float(camera.zoom), target, rel_tol=0.0, abs_tol=1e-9)
            after = (float(camera.offset_x), float(camera.offset_y), float(camera.zoom))
            if after != before:
    ...
    if callable(original_cleanup):
        def _cleanup() -> None:
            if isinstance(getattr(harness, "_fog_presentation_bounds_experiment_state", None), dict):
                _stop()
            original_cleanup()

        harness.cleanup = _cleanup

    harness._fog_presentation_bounds_experiment_installed = True
    return True
