"""Harness adapter for the final directed Fog raster-phase experiment."""
...
from .fog_directed_phase_generalization import DirectedPhaseRasterCollector

EXPERIMENT_ID = "fog_directed_phase_generalization_v1"
...
def install_fog_directed_phase_experiment(harness, world, profiler) -> bool:
    """Install isolated start/status/stop commands on the scale harness."""
    if bool(getattr(harness, "_fog_directed_phase_experiment_installed", False)):
        return True
    ...
    harness._fog_directed_phase_experiment_state = None
    ...
    def _start(command: Dict[str, Any]) -> Dict[str, Any]:
        if isinstance(
            getattr(harness, "_fog_directed_phase_experiment_state", None), dict
        ):
            return {"ok": False, "error": "fog_directed_phase_already_active"}
        if isinstance(getattr(harness, "_fog_camera_attribution_state", None), dict):
            return {"ok": False, "error": "fog_camera_attribution_active"}
        if isinstance(getattr(harness, "_scale_camera_stress_state", None), dict):
            return {"ok": False, "error": "terrain_camera_stress_active"}

        direction_name = str(command.get("direction", "")).strip().lower()
        ...
        state: Dict[str, Any] = {
            ...
            "direction_vector": DIRECTIONS[direction_name],
            "target_pixels": PAN_TARGET_PIXELS,
            "start_camera": {"offset_x": start_x, "offset_y": start_y, "zoom": zoom},
            "original_camera": {
        ...
        def _observe(**kwargs) -> None:
            ...
            state["anchor_ready"] = collector.initial_seeded
        ...
        camera.zoom = zoom
        presenter.reset()
        harness._fog_directed_phase_experiment_state = state
        return {
            "ok": True,
            "experiment": EXPERIMENT_ID,
            "direction": direction_name,
            "direction_vector": list(DIRECTIONS[direction_name]),
            "camera_start": dict(state["start_camera"]),
            "camera_original": dict(state["original_camera"]),
            "pan_target_pixels": PAN_TARGET_PIXELS,
    ...
    def _status() -> Dict[str, Any]:
        state = getattr(harness, "_fog_directed_phase_experiment_state", None)
        ...
        return {
            ...
            "canonical_camera_frames": len(state["collector"].frames),
    ...
    def _stop() -> Dict[str, Any]:
        state = getattr(harness, "_fog_directed_phase_experiment_state", None)
        if not isinstance(state, dict):
            return {"ok": False, "error": "fog_directed_phase_not_active"}
        camera = _camera()
        presenter = _find_fog_presenter(world)
        end_camera = (
            {
        ...
        response = {
            ...
            "direction_vector": list(state["direction_vector"]),
            "camera_start": dict(state["start_camera"]),
    ...
    def _handle(command: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if operation == "start_fog_directed_phase_generalization":
            return _start(command)
        if operation == "fog_directed_phase_generalization_status":
            return _status()
        if operation == "stop_fog_directed_phase_generalization":
            return _stop()
    ...
    if callable(original_update):
        ...
        def _update(delta_time: float) -> None:
            ...
            state = getattr(harness, "_fog_directed_phase_experiment_state", None)
            if not isinstance(state, dict) or not state["active"]:
                return
            if state["completed"] or not state["anchor_ready"]:
                return
            ...
            traveled_x = abs(float(camera.offset_x) - start["offset_x"])
            traveled_y = abs(float(camera.offset_y) - start["offset_y"])
            active_traveled = [
                value
                for value, direction in (
                    (traveled_x, direction_x),
                    (traveled_y, direction_y),
                )
                if direction
            ]
            remaining = max(0.0, state["target_pixels"] - max(active_traveled))
            speed = max(float(camera.speed), 1e-12)
            camera_dt = min(max(0.0, float(delta_time)), remaining / speed)
            delta = speed * camera_dt
            before = (float(camera.offset_x), float(camera.offset_y))
            camera.move(direction_x * delta, direction_y * delta)
            after = (float(camera.offset_x), float(camera.offset_y))
            if after != before:
                state["camera_changed_frames"] += 1
            traveled_x = abs(after[0] - start["offset_x"])
            traveled_y = abs(after[1] - start["offset_y"])
            state["completed"] = all(
                traveled >= state["target_pixels"]
                or math.isclose(
                    traveled, state["target_pixels"], rel_tol=0.0, abs_tol=1e-9
                )
                for traveled, direction in (
                    (traveled_x, direction_x),
                    (traveled_y, direction_y),
                )
                if direction
    ...
    if callable(original_cleanup):
        ...
        def _cleanup() -> None:
            if isinstance(
                getattr(harness, "_fog_directed_phase_experiment_state", None),
                dict,
            ):
                _stop()
            original_cleanup()
    ...
    harness._fog_directed_phase_experiment_installed = True
    return True
