...
def test_full_featured_occupancy_snapshot_replaces_per_unit_world_scan(monkeypatch):
    profiler = _Profiler()
    ...
    assert profiler.metrics["unit_full_occupancy_units"] == 3
    assert profiler.metrics["unit_full_occupancy_hexes"] == 2
    assert "unit_full_same_hex_lookup" in [name for name, _ in profiler.timers]
...
def test_unit_labels_use_prewarmed_quantized_surfaces_without_runtime_render(monkeypatch):
    ...
    assert renderer._frame_unit_label_cache_misses == 0
    assert draws and draws[0][1].center == (100, 200)
    assert "unit_full_icon_draw" in [name for name, _ in profiler.timers]
