...
def test_full_texture_cache_evicts_lru_and_caches_new_zoom_size(monkeypatch):
    ...
    assert renderer._frame_texture_scales == 1


def test_texture_cache_frame_metrics_expose_scaling_and_eviction(monkeypatch):
    renderer = _renderer_with_full_cache()
    metric_profiler = _MetricProfiler()
    monkeypatch.setattr(unit_render_module, "profiler", metric_profiler)
    monkeypatch.setattr(
        unit_render_module.pygame.transform,
        "scale",
        lambda original, size: f"scaled-{size[0]}",
    )

    renderer._get_cached_texture(Faction.WEI, UnitType.INFANTRY, 73)
    renderer._get_cached_texture(Faction.WEI, UnitType.INFANTRY, 73)
    renderer._publish_texture_cache_frame_stats()

    assert metric_profiler.metrics == {
        "unit_texture_cache_size": 3,
        "unit_texture_cache_hits": 1,
        "unit_texture_cache_misses": 1,
        "unit_texture_scales": 1,
        "unit_texture_cache_evictions": 1,
    }
