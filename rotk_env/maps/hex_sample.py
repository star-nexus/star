...
def _centered_cells(width: int, height: int) -> list[tuple[int, int]]:
    half_w = width // 2
    half_h = height // 2
    return [
        (j - half_w, half_h - i)
...
def sample_image(
    ...
    conv = HexConverter()
    cells = _centered_cells(width, height)
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Hex columns (default: existing file width, else 15)",
    )
    parser.add_argument(
        "--height",
        type=int,
        default=None,
        help="Hex rows (default: existing file height, else 15)",
    )
    ...
    width = args.width
    height = args.height
    if args.out.suffix == ".json" and args.out.exists():
        existing = load_map(args.out)
        formations = existing.formations
        formation_types = existing.formation_types
        unit_mix = existing.unit_mix
        name = existing.name
        if width is None:
    ...
    width = width or 15
    height = height or 15
    terrain = sample_image(image, width=width, height=height)
    if args.out.suffix == ".json":
        ...
        args.out.write_text(
            header + dump_ascii_map(terrain, width=width, height=height),
            encoding="utf-8",
        )
    print(f"Wrote {args.out}")
    if args.overlay:
        draw_overlay(image, terrain, args.overlay, width=width, height=height)
