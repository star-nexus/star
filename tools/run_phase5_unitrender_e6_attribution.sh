...
# Exact retained production is both control and treatment base. Treatment differs
# only by the measurement probe's per-hex geometry-reuse monkeypatch.
...
WARMUP="${WARMUP:-5}"
# With MovementAnimation.speed=2 tiles/s and a 12-step out-and-back route,
# one complete geometry-reuse cycle is ~12 s. The normal ~5 s rolling window
# saved at t=19 s therefore contains steady reuse rather than first-fill work.
SAMPLE_AFTER="${SAMPLE_AFTER:-19}"
READY_TIMEOUT="${READY_TIMEOUT:-120}"
COMMAND_TIMEOUT="${COMMAND_TIMEOUT:-120}"
COOLDOWN="${COOLDOWN:-5}"
RESULTS_ROOT="${RESULTS_ROOT:-results/phase5-unitrender-e6}"
RUN_ID="${RUN_ID:-$(date +%Y%m%d-%H%M%S)}"

REPO_ROOT="$(git rev-parse --show-toplevel)"
cd "$REPO_ROOT"
SCENARIO_FILE="$REPO_ROOT/rotk_env/maps/${SCENARIO}.json"
PROBE="$REPO_ROOT/tools/phase5_unitrender_e6_geometry_reuse.py"
CONTRACT_TEST="$REPO_ROOT/tools/test_phase5_unitrender_e6_geometry_reuse.py"
ANALYZER="$REPO_ROOT/tools/phase5_unitrender_e6_analyze.py"
...
git cat-file -e "${BASE_SHA}^{commit}" 2>/dev/null || {
    echo "ERROR: missing production base commit $BASE_SHA; run git fetch origin"
    exit 2
}
...
cleanup_env() {
    ...
    if [[ -n "$root" ]] && pid_alive "$root"; then
        ...
        echo "Stopping ENV process tree: root=$root descendants=${descendants//$'\n'/,}"
...
echo "Creating detached E6 control/treatment worktrees from retained production..."
...
cp "$PROBE" "$B_WT/tools/phase5_unitrender_e6_geometry_reuse.py"
cp "$CONTRACT_TEST" "$B_WT/tools/test_phase5_unitrender_e6_geometry_reuse.py"

{
    echo "experiment=Phase 5 UnitRender E6 derived world-geometry reuse attribution"
    echo "run_id=$RUN_ID"
    echo "created_at_utc=$(date -u '+%Y-%m-%dT%H:%M:%SZ')"
    echo "caller_branch=$(git branch --show-current)"
    echo "caller_sha=$(git rev-parse HEAD)"
    echo "production_base_sha=$BASE_SHA"
    echo "control_runtime_sha=$BASE_SHA"
    echo "treatment_runtime_sha=$BASE_SHA"
    echo "treatment=per_hex_derived_world_geometry_reuse_monkeypatch"
    ...
    echo "hub=offline"
    echo "mock_ai=off"
    echo "e5_3_world_xy_signal_ms_50=0.201"
    echo "e5_3_world_xy_signal_ms_100=0.375"
    ...
    echo "production_candidate_requires_bounded_geometry_ownership=true"
    echo "p99_diagnostic=true"
...
(
    cd "$B_WT"
    uv run --frozen pytest tools/test_phase5_unitrender_e6_geometry_reuse.py -q
...
run_point() {
    ...
    local launcher="$5"
    local point_dir="$RUN_DIR/$variant/${density_label}pct-moving"
    local cleanup_rc

    [[ -z "${ENV_PID:-}" ]] || { echo "ERROR: previous ENV still registered"; exit 2; }
    mkdir -p "$point_dir"
    CURRENT_SOCKET="/tmp/star-phase5-unitrender-e6-${$}-${variant}-${density_label}.sock"
    rm -f "$CURRENT_SOCKET"

    echo
    echo "============================================================"
    echo "UnitRender E6 attribution: ${variant} ${density_label}% moving"
    ...
    (
        ...
        if [[ "$launcher" == "treatment" ]]; then
            exec env STAR_SCALE_MINIMAP_UNITS=off \
              uv run --frozen python tools/phase5_unitrender_e6_geometry_reuse.py \
...
echo
echo "============================================================"
echo "UnitRender E6 attribution completed"
