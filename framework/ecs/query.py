...
class QueryFilter:
    ...
    def matches(self, entity_components: Set[Type[Component]]) -> bool:
        ...
        # 检查必须包含的组件
        if not all(
            comp_type in entity_components for comp_type in self.required_components
        ):
            return False

        # 检查必须排除的组件
        if any(
            comp_type in entity_components for comp_type in self.excluded_components
        ):
            return False

        # 检查至少包含其中一个的组件
        if self.any_of_components and not any(
            comp_type in entity_components for comp_type in self.any_of_components
        ):
            return False
...
class Query:
    ...
    def _execute_query(self) -> None:
        ...
        for entity in self.entity_manager.entities:
            # 获取实体的所有组件类型
            component_types = set(
                type(comp) for comp in self.component_manager.get_all_component(entity)
            )
        ...
        if self._order_by_func:
            results.sort(
                key=lambda entity: self._order_by_func(
    ...
    def iter_components(
        ...
        for entity in self.result():
            # 获取所需的组件
            components = tuple(
                self.component_manager.get_component(entity, comp_type)
...
class QueryBuilder:
    ...
    def where(
        self, predicate: Callable[[Entity, List[Component]], bool]
    ) -> "QueryBuilder":
        """
        添加自定义过滤条件
    ...
    def order_by(
        self, key_func: Callable[[Entity, List[Component]], Any]
    ) -> "QueryBuilder":
        """
        设置结果排序方式
