...
class EventManager:
    ...
    def publish(self, topic: str, message: Message = None) -> None:
        """发布事件
    ...
    def show_bus(self) -> None:
        """显示所有主题监听器"""
        for topic, subscribers in self._bus.items():
            print(f"{topic}: {subscribers}")

    def get_subscribers(self, topic: str) -> set:
        """获取订阅者列表

        Args:
            topic: 主题类型
        """
        return self._bus.get(topic, set())
